...
### Database Migration
```
$ docker exec -it laravel-php-fpm /bin/bash -c "php artisan migrate"
````

### Database Seeding
```
$ docker exec -it laravel-php-fpm /bin/bash -c "php artisan db:seed"
````

### Other Commands
```
$ docker exec -it laravel-php-fpm /bin/bash -c "echo Hello"
````

Same thing applies with other containers. Just change ```laravel-php-fpm``` to the container name that you want to access. The flag -c is for the command that you want to run.
